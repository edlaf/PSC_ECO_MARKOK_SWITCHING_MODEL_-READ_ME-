...
Y=[Amp(0.9,0.4,y)for y in X]
#plt.plot(X,Y)

Y2=[Amp(1,0.4,y)for y in X]
#plt.plot(X,Y2)

Y3=[Amp(0.8,0.4,y)for y in X]
#plt.plot(X,Y3)

Y4=[Amp(0.7,0.4,y)for y in X]
#plt.plot(X,Y4)

Y5=[Amp(0.6,0.4,y)for y in X]
#plt.plot(X,Y5)
df= pd.DataFrame({'X':X,'Y':Y,'Y2':Y2,'Y3':Y3,'Y4':Y4,'Y5':Y5})
print(df)
df.to_csv('courbe_tipe_edouard_XY.csv',sep=';',index=False)
#plt.show()
